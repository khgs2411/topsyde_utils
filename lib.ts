...
import { ENCRYPTION_PHRASE } from "../consts";
...
class Lib {
	 ...
	 *
	 * @returns Returns a unique key.
	 */
	public static GetUniqueKey(): string {
		return ENCRYPTION_PHRASE + Lib.UUID();
	}
	...
	public static GetLocaleFromPhone(number: string): {
		language: string;
		countryCodeISO2: string;
		valid: boolean;
	} {
		const result: PhoneResult = phone(this.FormatPhone(number));
		if (result.isValid) {
			const country = clm.getCountryByAlpha2(result.countryIso2);
			return {
				language: country?.languages[0] ?? "en",
				countryCodeISO2: result.countryIso2,
				valid: true,
			};
		}
		return { language: "en", countryCodeISO2: "US", valid: false };
	}
