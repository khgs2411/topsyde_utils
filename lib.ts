...
class Lib {
  ...
  public static GetDateTimestamp(
    date: string | Date,
  ...
  public static FormatUnixToDate(
    unix_time_stamp: number,
    in_milliseconds: boolean = false
  ): Date {
  ...
  public static DaysBetweenDates(
    ...
    if (
      ...
      throw new Error(
        "Lib.DaysBetweenDates() Exception: Dates are required" +
          startDate +
          " " +
          endDate
      );
  ...
  public static IsPastDate(
    ...
    const truncateToSecond = (d: Date) =>
      new Date(Math.floor(d.getTime() / 1000) * 1000);

    const now = truncateToSecond(
      currentDate
  ...
  public static IsPrimitive(value: any): boolean {
    return (
      this.IsNumber(value) || this.IsString(value) || this.IsBoolean(value)
    );
  }
  ...
  public static IsNumpty(value: any, _objectsOnly: boolean = false): boolean {
    return typeof value === "number"
      ? false
      : this.IsNil(value) || this.IsEmpty(value, _objectsOnly);
  }
  ...
  public static GetProjectRoot(
    ...
    rootReference: string = "package.json"
  ): string {
  ...
  public static async RunTaskWithTimeout(
    task: () => Promise<void>,
    timeout: number
  ) {
    return Promise.race([
  ...
  public static async measureExecutionTime<
